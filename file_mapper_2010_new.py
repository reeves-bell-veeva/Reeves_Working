...
def raw_title_filename_exact(row) -> int:
    """
    Return 1 if the *raw* title text exactly matches the file's basename (case-insensitive),
    otherwise 0. This helps distinguish things like 'Plan 1' vs 'Plan_1' that normalize the same.
    """
    t = row.get("title")
    f = row.get("filename_file")
    if not isinstance(t, str) or not isinstance(f, str):
        return 0

    # Normalize basic whitespace and case, but DO NOT collapse spaces/underscores
    t0 = _normalize_unicode_spaces(t).strip().lower()
    f0 = _normalize_unicode_spaces(f).strip().lower()

    # Strip extension from filename
    if "." in f0:
        f0 = f0.rsplit(".", 1)[0]

    return 1 if t0 == f0 else 0
...
def folder_align_score(folder_token: str, file_norm_path: str) -> int:
    """
    Return 1 if the normalized folder_token is present in the file's normalized path,
    else 0. This lets us prefer the path whose folder best aligns to main.full_path.
    """
    if not isinstance(folder_token, str) or not folder_token.strip():
        return 0
    if not isinstance(file_norm_path, str) or not file_norm_path.strip():
        return 0
    ft = normalize_folder(folder_token)
    return 1 if ft and ft in file_norm_path else 0
...
# A1) Exact
pass1 = main.merge(
    files_df[["path", "filename_file", "norm_filename", "date_key", "norm_path"]],
    left_on="norm_title",
...
idx = files_df[["norm_filename", "path", "filename_file", "date_key", "norm_path"]].drop_duplicates().reset_index(drop=True)
...
stageA = pd.concat(
    [
        pass1[["document_id", "title", "path", "filename_file", "match_type", "intra_score", "norm_path"]],
        pass2[["document_id", "title", "path", "filename_file", "match_type", "intra_score", "norm_path"]],
    ],
    ignore_index=True,
)

# attach folder_token from main (based on document_id)
stageA = stageA.merge(
    main[["document_id", "folder_token"]],
    on="document_id",
    how="left",
)


stageA["folder_score"] = stageA.apply(
    lambda r: folder_align_score(r.get("folder_token"), r.get("norm_path")),
    axis=1,
)

stageA["raw_exact_score"] = stageA.apply(raw_title_filename_exact, axis=1)

# One-to-one within Stage A
stageA_best = stageA.sort_values(
    ["raw_exact_score", "folder_score", "match_type", "intra_score"],
...
# Bring through the main full_path for reporting
if "full_path" not in final_matches.columns:
    final_matches = final_matches.merge(
        main[["document_id", "full_path"]],
        on="document_id",
        how="left",
    )
...
unmatched_listing["full_path"] = unmatched_listing.get("full_path", unmatched_listing["path"])
cols_unmatched_listing = [c for c in ["path","filename_file","kind","norm_path","full_path"] if c in unmatched_listing.columns]
...
# -----------------------------
final_cols = ["document_id","title","full_path","path","filename_file","match_type","intra_score"]
